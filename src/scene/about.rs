...
impl SceneAbout {
    pub fn new(common_data: CommonData, ctx: &mut dyn Context) -> Result<SceneAbout> {
        let view = ctx.view().get_or_default();

        let text = match cake_engine::fs::read_lang_asset_to_string("about.txt") {
            Ok(mut text) => {
                text.truncate(text.trim_end().len());
                text
            }
            Err(e) => {
                cake_engine::log::error!("Failed to read about: {:?}", e);
                r"¯\(°_o)/¯".to_string()
            }
        };
